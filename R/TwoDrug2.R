...
SYN.test <- function(Drug1=Drug1, Drug2=Drug2, location4, r=5)
{
  ...
  Comb <- read.table(file = location4, header = T)
  for (i in 1:length(Comb[,3])) {
    if (Comb[i,3]>100) {
      Comb[i,3]<-100
    }
  }
...
SYN.test<-SYN.test(Drug1=Drug1, Drug2=Drug2, location4, r=r)
#In the example Combination Dataset, r=6


SYN.analysis <- function(Drug1=Drug1, Drug2=Drug2, Combination=location4, r=r, method = "loglog")
{
 ...
 SYN.analysis <- SYN.analysis(Drug1=Drug1, Drug2=Drug2, Combination=Combination, r=6, method = method)
